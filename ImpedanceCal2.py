...
class PerformanceOptimizer:
    ...
    def parallel_parameter_sweep(parameter_ranges: Dict, calc_function, n_processes: int = 4):
        ...
        try:
            ...
            print("Multiprocessing not available, running sequentially")
        ...
        print(f"Running {len(combinations)} calculations...")
        ...
        if multiprocessing_available and n_processes > 1:
            ...
            results = [calculate_single(combo) for combo in combinations]
    ...
    def create_design_optimization_plot(sweep_results: List[Dict], 
        ...
        if len(width_unique) > 1 and len(height_unique) > 1:
            ...
            for i, w in enumerate(width_unique):
                for j, h in enumerate(height_unique):
                    # Find matching result
                    for result in valid_results:
                        if (abs(result['parameters']['width']*1e3 - w) < 0.001 and 
                            abs(result['parameters']['height']*1e3 - h) < 0.001):
                            Z[j, i] = result.get('Z0_analytical', result.get('Z0_fem', 0))
                            break
            
            contour = ax1.contourf(W, H, Z, levels=20, cmap='viridis')
            ax1.contour(W, H, Z, levels=[target_impedance], colors='red', linewidths=2)
            plt.colorbar(contour, ax=ax1, label='Impedance (Ω)')
            ax1.set_xlabel('Width (mm)')
            ax1.set_ylabel('Height (mm)')
            ax1.set_title(f'Impedance Design Space (Target: {target_impedance}Ω)')
        
        # Error analysis plot
        errors = [abs(r.get('Z0_analytical', 0) - target_impedance) for r in valid_results]
        scatter = ax2.scatter(widths, heights, c=errors, cmap='Reds', s=50)
        plt.colorbar(scatter, ax=ax2, label='Error (Ω)')
        ax2.set_xlabel('Width (mm)')
        ax2.set_ylabel('Height (mm)')
        ax2.set_title('Impedance Error from Target')
...
class TransmissionLineCalculator:
    ...
    def solve_eigenproblem(self, K_matrix: np.ndarray, M_matrix: np.ndarray, 
        ...
        try:
            ...
            eigenvectors = np.random.random((K_matrix.shape[0], n_modes))
    ...
    def _calculate_electric_energy(self, field: np.ndarray, nodes: np.ndarray, 
        ...
        try:
            field_2d = field.reshape(int(np.sqrt(len(field))), -1)
            E_field = np.gradient(field_2d)
            energy = 0.5 * epsilon_r * np.sum([np.sum(e**2) for e in E_field])
        except:
            energy = np.sum(field**2) * epsilon_r
        return energy
    ...
    def _calculate_magnetic_energy(self, field: np.ndarray, nodes: np.ndarray) -> float:
        ...
        try:
            field_2d = field.reshape(int(np.sqrt(len(field))), -1)
            H_field = np.gradient(field_2d)
            energy = 0.5 * np.sum([np.sum(h**2) for h in H_field])
        except:
...
def create_interactive_widgets():
    """Create interactive widgets for transmission line calculation."""
    ...
    width_widget = widgets.FloatText(
        value=0.1, description='Trace Width (mm):', 
        style=style, layout=layout, step=0.01
    )
    
    height_widget = widgets.FloatText(
        value=0.2, description='Dielectric Height (mm):', 
        style=style, layout=layout, step=0.01
    )
    ...
    thickness_widget = widgets.FloatText(
        value=35, description='Copper Thickness (μm):', 
        style=style, layout=layout, step=1
    )
    
    # Differential parameters
    spacing_widget = widgets.FloatText(
        value=0.1, description='Trace Spacing (mm):', 
        style=style, layout=layout, step=0.01
    )
    ...
    def on_calculate_click(b):
        ...
        with output:
            ...
            # Get widget values and convert to meters
            width = width_widget.value * 1e-3  # Convert mm to meters
            height = height_widget.value * 1e-3
            epsilon_r = epsilon_widget.value
            thickness = thickness_widget.value * 1e-6  # Convert μm to meters
            spacing = spacing_widget.value * 1e-3
...
def export_results_to_excel(results: Dict, filename: str = "impedance_results.xlsx"):
    """Export calculation results to Excel file."""
    try:
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            # Single-ended results
            if 'Z0_analytical' in results:
                se_data = {
                    'Parameter': ['Analytical Impedance', 'FEM Impedance', 'Effective Permittivity'],
                    'Value': [results['Z0_analytical'], results['Z0_fem'], results['eps_eff']],
                    'Unit': ['Ω', 'Ω', '-']
                }
                se_df = pd.DataFrame(se_data)
                se_df.to_excel(writer, sheet_name='Single-Ended', index=False)
            
            # Differential results
            if 'Z_diff_analytical' in results:
                diff_data = {
                    'Parameter': ['Differential Impedance (Analytical)', 'Differential Impedance (FEM)', 
                                'Common Mode Impedance', 'Coupling Factor'],
                    'Value': [results['Z_diff_analytical'], results['Z_diff_fem'], 
                            results['Z_common'], results['coupling_factor']],
                    'Unit': ['Ω', 'Ω', 'Ω', '-']
                }
...
def design_optimization_example():
    ...
    # Define parameter ranges for 50Ω target
    parameter_ranges = {
        'width': np.linspace(0.05e-3, 0.3e-3, 10),    # 0.05-0.3mm
        'height': np.linspace(0.1e-3, 0.5e-3, 10),    # 0.1-0.5mm
        'epsilon_r': [4.4],                           # FR4
        'thickness': [35e-6]                          # 35μm copper
    }
    
    def calc_function(width, height, epsilon_r, thickness):
        """Calculation function for parameter sweep."""
        return calc.calculate_microstrip_impedance(width, height, epsilon_r, thickness)
    
    # Run parameter sweep
    print("Running design optimization sweep...")
    results = optimizer.parallel_parameter_sweep(parameter_ranges, calc_function, n_processes=2)
    
    # Create optimization plots
    fig = optimizer.create_design_optimization_plot(results, target_impedance=50.0)
    if fig:
        plt.show()
    
    # Find best match to 50Ω
    valid_results = [r for r in results if 'error' not in r]
    errors = [abs(r['Z0_analytical'] - 50.0) for r in valid_results]
    best_idx = np.argmin(errors)
    best_result = valid_results[best_idx]
    
    print(f"\nBest 50Ω design:")
    print(f"Width: {best_result['parameters']['width']*1e3:.3f} mm")
    print(f"Height: {best_result['parameters']['height']*1e3:.3f} mm")
    print(f"Impedance: {best_result['Z0_analytical']:.2f} Ω")
    print(f"Error: {errors[best_idx]:.2f} Ω")
...
def run_advanced_examples():
    ...
    print("\nHigh-Frequency Microstrip Analysis:")
    calc = TransmissionLineCalculator()
    calc.mesh_density = 80  # Higher density for accuracy
    
    results_hf = calc.calculate_microstrip_impedance(
        width=0.05e-3,      # Very narrow trace
        height=0.1e-3,      # Thin dielectric
        epsilon_r=3.5,      # Low-loss material
        thickness=18e-6     # Thin copper
    )
    
    print(f"Narrow trace impedance: {results_hf['Z0_analytical']:.1f}Ω")
    print(f"Effective εr: {results_hf['eps_eff']:.3f}")
    
    # Design optimization example
    print("\nRunning design optimization example...")
    try:
...
if __name__ == "__main__":
    ...
    print("Creating interactive widget interface...")
